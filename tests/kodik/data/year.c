...
extern
kodik_year_t *
kodik_year_new_from_json(json_t const *);
...
main(int argc, char *argv[]) {
  ...
  kodik_year_t *kodik_object = NULL;
  size_t index = 0;

  json_array_foreach(json_results, index, json_cursor) {
    kodik_object = kodik_year_new_from_json(json_cursor);
    munit_assert_ptr_not_null(kodik_object);

    munit_assert_int64(
      kodik_year_get_count(kodik_object),
      ==,
      json_integer_value(json_object_get(json_cursor, "count"))
    );

    munit_assert_int64(
      kodik_year_get_year(kodik_object),
      ==,
      json_integer_value(json_object_get(json_cursor, "year"))
    );

    fprintf (stdout, "%s\n", json_dumps(json_cursor, JSON_COMPACT));

    kodik_year_free(kodik_object);
    kodik_object = NULL;
  }
