...
kodik_v2_translation_new_from_json(json_object const *);

extern
void
kodik_v2_translation_free(struct kodik_v2_translation *);

UTEST(kodik, v2_translation) {
  ...
  for (size_t i = 0; i < array_size; i++) {
    ...
    struct kodik_v2_translation *v2_translation = kodik_v2_translation_new_from_json(current);
    ASSERT_NE(NULL, v2_translation);

    json_object *j_id,
                *j_count,
                *j_title;

    ASSERT_TRUE(json_object_object_get_ex(current, "id", &j_id));
    ASSERT_TRUE(json_object_object_get_ex(current, "title", &j_title));
    ASSERT_TRUE(json_object_object_get_ex(current, "count", &j_count));

    ASSERT_EQ(kodik_v2_translation_get_id(v2_translation), json_object_get_int64(j_id));
    ASSERT_EQ(kodik_v2_translation_get_count(v2_translation), json_object_get_int64(j_count));
    ASSERT_STREQ(kodik_v2_translation_get_title(v2_translation), json_object_get_string(j_title));

    fprintf(stdout, "%s\n", json_object_to_json_string_ext(current, JSON_C_TO_STRING_PRETTY));

    kodik_v2_translation_free(v2_translation);
  }
