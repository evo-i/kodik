...
UTEST(kodik, genre) {
  ...
  for (size_t i = 0; i < array_size; i++) {
    ...
    ASSERT_EQ(kodik_genre_get_count(genre), json_object_get_int64(j_count));
    ASSERT_STREQ(kodik_genre_get_title(genre), json_object_get_string(j_title));

    fprintf(stdout, "%s\n", json_object_to_json_string_ext(current, JSON_C_TO_STRING_PRETTY));

    kodik_genre_free(genre);
  }
